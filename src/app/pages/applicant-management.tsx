...
import { Upload, Trophy, Award, FileText, Download, CheckCircle, XCircle, AlertCircle, X, Upload as UploadIcon } from 'lucide-react';
...
interface Applicant {
  ...
  status: string;
  hiringStatus?: 'Selected' | 'Rejected' | 'Pending';
}
...
export default function ApplicantManagement() {
  ...
  // Get display status based on hiring decision and evaluation status
  const getDisplayStatus = (applicant: Applicant) => {
    if (applicant.hiringStatus === 'Selected') return 'Selected';
    if (applicant.hiringStatus === 'Rejected') return 'Rejected';
    return applicant.status;
  };

  const handleHiringDecision = (decision: 'Selected' | 'Rejected') => {
    if (selectedApplicant) {
      const updatedApplicants = applicants.map((a) =>
        a.id === selectedApplicant.id ? { ...a, hiringStatus: decision } : a
      );
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-start justify-between mb-2">
                        ...
                        {applicant.hiringStatus && applicant.hiringStatus !== 'Pending' && (
                          <span
                            className={`text-xs px-2 py-1 rounded ${
                              applicant.hiringStatus === 'Selected'
                                ? 'bg-green-100 text-green-700'
                                : applicant.hiringStatus === 'Rejected'
        ...
        {/* Right Column - Evaluation and Hiring Decision Panel */}
            ...
            <>
              ...
              <div className="bg-white rounded-lg shadow-md p-6 border border-gray-100">
                <div className="flex items-center justify-between mb-4">
                  ...
                  {selectedApplicant.hiringStatus && (
                    <span
                      className={`px-4 py-2 rounded-lg text-sm font-medium ${
                        selectedApplicant.hiringStatus === 'Selected'
                          ? 'bg-green-100 text-green-700'
                          : selectedApplicant.hiringStatus === 'Rejected'
                          ? 'bg-red-100 text-red-700'
                          : 'bg-gray-100 text-gray-700'
                      }`}
                    >
              ...
              <div className="bg-white rounded-lg shadow-md p-6 border border-gray-100">
                <h3 className="text-lg text-gray-900 mb-4">Hiring Decision</h3>
                <p className="text-sm text-gray-600 mb-4">
                  Review the evaluation results and make a hiring decision for this applicant.
                </p>
                <div className="flex gap-4">
                  <button
                    onClick={() => handleHiringDecision('Selected')}
                    disabled={selectedApplicant.hiringStatus === 'Selected'}
                  ...
                  <button
                    ...
                    disabled={selectedApplicant.hiringStatus === 'Rejected'}
                ...
                {selectedApplicant.hiringStatus !== 'Pending' && (
                  <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                    <p className="text-sm text-blue-800">
                      <strong>Status:</strong> This applicant has been{' '}
                      {selectedApplicant.hiringStatus === 'Selected' ? 'approved for hiring' : 'rejected'}.
                    </p>
                  </div>
                )}
              </div>
            ...
            <form onSubmit={(e) => { e.preventDefault(); alert('Application uploaded!'); setShowUploadModal(false); }} className="p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      required
                      placeholder="40 hours relevant training"
                      className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#C62828] focus:border-transparent"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      required
                      placeholder="3 years relevant experience"
                      className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#C62828] focus:border-transparent"
                    />
                  </div>
                </div>
              ...
              <div className="mb-6">
                ...
                <div className="space-y-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">
                      Resume / Curriculum Vitae <span className="text-red-500">*</span>
                    </label>
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">
                      Certificates (Training, Eligibility, etc.) <span className="text-red-500">*</span>
                    </label>
                    <div className="border-2 border-dashed border-gray-300 rounded-lg p-4 text-center hover:border-[#C62828] transition-colors cursor-pointer">
                      <UploadIcon className="w-8 h-8 text-gray-400 mx-auto mb-2" />
                      <p className="text-sm text-gray-600">Click to upload or drag and drop</p>
                      <p className="text-xs text-gray-500">PDF, DOCX (Max 5MB)</p>
                      <input type="file" accept=".pdf,.docx" className="hidden" multiple />
                    </div>
                  </div>
                </div>
              </div>
